...
/***/ function(module, exports, __webpack_require__) {
	...
	var displayAction = function displayAction(action, player, tile) {

	  var html = '<p class="game-message">';
	  switch (action) {
	    case 'win':
	...
	var displayResults = function displayResults(data) {
	  var html = '<table class="result-table"><tr><th>Game ID</th><th>Finished?</th></tr>';
	...
	var displayResult = function displayResult(data) {
	  var html = '<p class="search-result">Game with ID ' + data.game.id + 'was ';
	  if (data.game.over) {
	...
	var displayStats = function displayStats(data) {
	  var html = '<p class="search-result">Stats: You have played ' + data.games.length + ' games, of which ';
	  var count = 0;
	  for (var i = 0; i < data.games.length; i++) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onGetGames = function onGetGames(event) {
	  ...
	  if (gameId.length === 0) {
	    ...
	    api.show(gameId).then(ui.onSuccess).catch(ui.onError);
	  }
	};
	...
	var onGetStats = function onGetStats(event) {
	  ...
	  api.index().then(ui.onStatSuccess).catch(ui.onError);
	};
	...
	var onCreateGame = function onCreateGame(event) {
	  ...
	  api.create().then(function (response) {
	    ...
	    return store.game;
	  }).then(ui.onCreateSuccess).catch(ui.onError);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(11);
	var output = __webpack_require__(8);
	...
	var onStatSuccess = function onStatSuccess(data) {
	  // console.table(data.games);
	...
	var onCreateSuccess = function onCreateSuccess(data) {
	  // console.log(data.game);
	  var game = new engine.Game(store.game.cells, store.game.id);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var insertImg = function insertImg(target, piece) {
	  var html = '<img src="';
	  if (piece === 'x') {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var initGame = function initGame(bindMakeMove) {
	...
	var drawBoard = function drawBoard(game) {
	  ...
	  // need to bind `this` for certain methods
	  var bindMakeMove = game.makeMove.bind(game);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var states = __webpack_require__(16);
	var ui = __webpack_require__(8);
	var api = __webpack_require__(10);
	var utils = __webpack_require__(13);
	...
	Game.prototype.checkWinState = function (board, piece) {
	  ...
	  var horizontal = states.checkHorizontalStates(board, piece);
	  var vertical = states.checkVerticalStates(board, piece);
	  var diagonal = states.checkDiagonalStates(board, piece);
	...
	Game.prototype.updateRemote = function (i, player, gameOver) {
	  ...
	  api.update(this.id, data).then(ui.onSuccess).catch(ui.onError);
	};
	...
	Game.prototype.makeMove = function (event) {
	  ...
	  if (this.board[index] === '') {
	    ...
	    var win = this.checkWinState(this.board, this.currentPlayer);
	    if (win) {
	      ...
	      //if game is still going, change turn
...
/***/ function(module, exports) {
	...
	var checkHorizontalStates = function checkHorizontalStates(gameBoard, piece) {
	  ...
	  var firstRow = gameBoard.slice(0, 3).every(function (e) {
	    return e === piece;
	  });
	  var secondRow = gameBoard.slice(3, 6).every(function (e) {
	    return e === piece;
	  });
	  var thirdRow = gameBoard.slice(6).every(function (e) {
	    return e === piece;
	  });
	...
	var checkVerticalStates = function checkVerticalStates(gameBoard, piece) {
	  ...
	  // this could all be done in the return but this reads easier
	  var first = firstColumn.every(function (e) {
	    return e === piece;
	  });
	  var second = secondColumn.every(function (e) {
	    return e === piece;
	  });
	  var third = thirdColumn.every(function (e) {
	    return e === piece;
	  });
	...
	var checkDiagonalStates = function checkDiagonalStates(gameBoard, piece) {
	  ...
	  // this could all be done in the return but this reads easier
	  var first = firstDiagonal.every(function (e) {
	    return e === piece;
	  });
	  var second = secondDiagonal.every(function (e) {
	    return e === piece;
	  });
	...
	module.exports = {
	  checkHorizontalStates: checkHorizontalStates,
	  checkVerticalStates: checkVerticalStates,
	  checkDiagonalStates: checkDiagonalStates
	};
